...
function buildFragments(room) {
  if (room===0) {
    fragments = [
      { id:0, x:420,  y:groundY-80,  collected:false, bobPhase:0   },
      { id:1, x:900,  y:groundY-120, collected:false, bobPhase:1.2 },
      { id:2, x:1400, y:groundY-90,  collected:false, bobPhase:2.4 },
    ];
  } else {
    fragments = [
...
// ─── PHYSICS ───
const GRAV=0.44, SPEED=3.0, JUMP=-10;
function updatePlayer() {
  ...
  if(L){player.vx=-SPEED; player.dir=-1;}
  if(R){player.vx= SPEED; player.dir= 1;}
...
function collectFrag(f) {
  ...
  setTimeout(()=>fl.classList.remove('show'),1000);
...
function drawFragments() {
  ...
  fragments.forEach(f=>{
    ...
    if(dist<70){
      ...
      ctx.font='13px "Noto Serif KR",serif';
      ctx.fillStyle=currentRoom===1?'rgba(240,205,140,0.9)':'rgba(200,235,255,0.9)';
      ctx.textAlign='center'; ctx.fillText(FRAG_WORDS[f.id],fx,fy-22); ctx.restore();
    }
  });
}
